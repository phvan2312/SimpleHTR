...
def add_timing_signal(x, max_height, hid_dim, n_layers=1, keep_prob=0.8):
    N = tf.shape(x)[0]
    H, W = x.shape[1].value, tf.shape(x)[2]
    C = x.shape[3].value

    # initializing cell
    pos_E = tf.get_variable(shape=[max_height, C], name="pos_E", dtype=tf.float32,
                            initializer=tf.contrib.layers.variance_scaling_initializer())
    cell = [DropoutWrapper(LSTMCell(num_units=hid_dim), output_keep_prob=keep_prob) for _ in range(n_layers)]
    cell = MultiRNNCell(cell)

    # loop through time-step
    # inputs_by_time = tf.unstack(x, axis=2)
    timesteps = W

    def forward(t):
        inp_t = x[:, :, t, :]  # (batch_size, height , channels)
        pos_t = tf.expand_dims(tf.nn.embedding_lookup(pos_E, tf.fill([N, ], t)), axis=1)  # (batch_size, 1, channels)
        inp = tf.concat([pos_t, inp_t], axis=1)

        # output_t \in (batch_size, height + 1, hidden_dim), state_t \in size (batch_size, hidden_dim)
        output_t, state_t = tf.nn.dynamic_rnn(cell, inp, initial_state=cell.zero_state(batch_size=N, dtype=tf.float32))

        return output_t, state_t[-1].h

    condition = lambda t, _, __: tf.less(t, timesteps)

    def step(t, outputs, states):
        output_t, state_t = forward(t)
        state_t = tf.expand_dims(state_t, axis=1)

        return [tf.add(t, 1), tf.concat([outputs, output_t], 1), tf.concat([states, state_t], 1)]

    init_outputs, init_states = forward(0)
    init_states = tf.expand_dims(init_states, axis=1)

    _, outputs, states = tf.while_loop(cond=condition, body=step, loop_vars=[tf.constant(1), init_outputs, init_states],
                                       shape_invariants=
                                       [tf.constant(0).get_shape(), tf.TensorShape([None, None, hid_dim]),
                                        tf.TensorShape([None, None, hid_dim])],
                                       )
...
class Model:
    ...
    def setupCNN(self, cnnIn3d):
        ...
        kernelVals = [5, 3, 3]
        featureVals = [1, 32, 64, 128]
        strideVals = poolVals = [(2, 2), (1, 2), (1, 2)]
        ...
        pool = tf.transpose(pool, perm=[0,2,1,3])
        _, pool = add_timing_signal(pool,200,hid_dim=64,n_layers=1,keep_prob=1.0)
        pool = tf.expand_dims(pool,2)
